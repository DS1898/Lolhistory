...
export default function StreamerPage() {
  ...
  async function fetchData() {
    ...
    // 0. 해당 스트리머가 참여한 시즌 목록
    const { data: seasonData } = await supabase
      .from('match_participants')
      .select('match_id, matches!inner(season)')
      .eq('streamer_id', id);
    const uniqueSeasons = [...new Set((seasonData || []).map(p => p.matches?.season).filter(Boolean))].sort((a,b)=>b-a);
    setAvailableSeasons(uniqueSeasons);
    if (uniqueSeasons.length > 0 && !uniqueSeasons.includes(season)) {
    ...
    const { data: myParts } = await supabase
      .from('match_participants')
      .select('id, team, champion_id, champion_level, position, kills, deaths, assists, result, match_id, spell1_id, spell2_id, rune_keystone, rune_secondary, item0, item1, item2, item3, item4, item5, item6, item7')
      .eq('streamer_id', id)
    ...
    if (recentMatchIds.length > 0) {
      const { data: allParts } = await supabase
        .from('match_participants')
        .select('id, match_id, streamer_id, team, champion_id, position, champion_level, spell1_id, spell2_id, rune_keystone, rune_secondary, item0, item1, item2, item3, item4, item5, item6, item7, result, kills, deaths, assists, streamer:streamers(id, name, profile_image_url)')
  ...
  useEffect(() => { initDDragon(); fetchData(); }, [id, season]);
