...
export default function InquiriesPage() {
  ...
  async function handleReply(inq) {
    const reply = (replyDrafts[inq.id] ?? inq.reply ?? '').trim();
    if (!reply) return;
    setSavingId(inq.id);
  ...
  return (
    <div style={{ maxWidth: 860 }}>
      ...
      <div className="bg-bg-card border border-border rounded-xl overflow-hidden">
        {/* 테이블 헤더 */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px 100px 130px' }}
          className="px-4 py-3 border-b border-border text-xs font-semibold text-text-muted uppercase tracking-wide">
          <span>이름 / 내용</span><span>등록일</span><span>상태</span><span>액션</span>
        </div>
        ...
        {loading ? (
          ...
          <div key={inq.id} className="border-b border-border last:border-b-0">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 100px 100px 130px' }}
              ...
              {/* 이름 + 아코디언 토글 */}
              <button onClick={() => handleExpand(inq.id)}
                ...
                <div className="min-w-0">
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span className={`text-sm font-medium truncate ${!inq.is_read ? 'text-text-primary' : 'text-text-secondary'}`}>
                      {inq.name}
                    </span>
                    {inq.reply && (
                  ...
                  <p className="text-xs text-text-muted truncate mt-0.5">{inq.message}</p>
                </div>
              </button>

              <span className="text-xs text-text-muted">{relDate(inq.created_at)}</span>

              <button onClick={() => toggleRead(inq)}
                className={`text-xs font-semibold px-2 py-1 rounded-full w-fit transition-colors ${!inq.is_read ? 'bg-win/15 text-win' : 'bg-bg-input text-text-muted border border-border'}`}>
                {!inq.is_read ? '미읽음' : '읽음'}
              </button>
            ...
            {/* 아코디언 내용 + 답변 입력 */}
            {expandedId === inq.id && (
              <div style={{ background: 'rgba(255,255,255,0.018)', borderTop: '1px solid var(--border-clr)', padding: '16px 20px 16px 44px' }}>
                {/* 문의 내용 */}
                <div style={{ marginBottom: 16 }}>
                  <p style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-lo)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    문의 내용
                  </p>
                  <p style={{ fontSize: '0.85rem', lineHeight: 1.8, color: 'var(--text-mid)', whiteSpace: 'pre-wrap', wordBreak: 'break-word', margin: 0, padding: '10px 14px', background: 'var(--bg-input)', borderRadius: 8, border: '1px solid var(--border-clr)' }}>
                    {inq.message}
                  </p>
                  <p style={{ fontSize: '0.72rem', color: 'var(--text-lo)', marginTop: 6 }}>
                    등록: {new Date(inq.created_at).toLocaleString('ko-KR')}
                  </p>
                </div>
                ...
                {inq.reply && !(inq.id in replyDrafts) && (
                  <div style={{ marginBottom: 14 }}>
                    <p style={{ fontSize: '0.7rem', fontWeight: 700, color: '#22c55e', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                      답변 완료 · {inq.replied_at ? relDate(inq.replied_at) : ''}
                    </p>
                    <p style={{ fontSize: '0.85rem', lineHeight: 1.8, color: 'var(--text-mid)', whiteSpace: 'pre-wrap', wordBreak: 'break-word', margin: 0, padding: '10px 14px', background: 'rgba(34,197,94,0.06)', borderRadius: 8, border: '1px solid rgba(34,197,94,0.2)' }}>
                      {inq.reply}
                    </p>
                ...
                {/* 답변 입력 폼 (답변 없거나 수정 중일 때) */}
                {(!inq.reply || inq.id in replyDrafts) && (
                  <div>
                    <p style={{ fontSize: '0.7rem', fontWeight: 700, color: '#4489c8', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    ...
                    <textarea
                      ...
                      style={{
                        ...
                        color: 'var(--text-hi)', outline: 'none', lineHeight: 1.7,
                        fontFamily: 'inherit',
                      }}
                    />
                    <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                      <button
                        ...
                        style={{
                          ...
                          cursor: savingId === inq.id ? 'not-allowed' : 'pointer',
